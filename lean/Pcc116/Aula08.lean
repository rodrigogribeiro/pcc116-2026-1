...
def iapp {A : Type} 
  ...
  | 0,     n, .inil, ys => by 
      rw [Nat.zero_add] 
      exact ys 
  | m + 1, n, .icons x xs, ys => by 
      have h : m + n + 1 = m + 1 + n := by omega
      rw [← h] 
      exact (.icons x (iapp xs ys))
...
#eval eject (iapp (.icons 1 (.icons 2 .inil)) 
                  (.icons 3 .inil))  -- [1, 2, 3]
...
def lista123 : IList ℕ 3 := 
  .icons 10 (.icons 20 (.icons 30 .inil))

#eval iget lista123 .zero
#eval iget lista123 (.succ .zero)
#eval iget lista123 (.succ (.succ .zero))
-- #eval iget lista123 (.succ (.succ (.succ .zero)))

/- Listas Heterogêneas -/

universe u v

inductive HList {α : Type u} 
                (B : α → Type v) 
                : List α → Type (max u v) where
  | hnil  : HList B []
  | hcons {t : α} 
          {ts : List α} 
          : B t → 
...
def esquema : List TipoBase := [.TNat, .TBool, .TString
                               , .TNat]

def registro : HList denotaBase esquema :=
  .hcons 42 (.hcons true (.hcons "Lean" 
              (.hcons 12 .hnil)))
...
def htl {α : Type u} 
        {B : α → Type v} 
        {t : α} 
        {ts : List α} : HList B (t :: ts) → HList B ts
  | .hcons _ xs => xs
...
def registroF : FHList denotaBase esquema :=
  (42, (true, ("Lean", ())))
...
def esquema2 : List TipoBase := [.TNat]
def registro2 : HList denotaBase esquema2 := 
  .hcons 100 .hnil

-- O índice .tail^3 .head acessa a 4.ª posição (primeiro elemento de registro2)
#eval hget (happ registro registro2) 
           (.tail (.tail (.tail .head)))  -- 100
