...
- Apresentar os sistemas de dedução natural e cálculo de sequentes para a lógica
  de predicados.
...
- Para isso, vamos revisar a sintaxe, semântica e os formalismos de dedução
...
- O conjunto de termos sobre uma assinatura $\Sigma$:
  ...
  - Se $f \in \mathcal{T}_{\Sigma}$, $a(f) = n$ e
    $t_1 ... t_n \in \mathcal{T}_{\Sigma}$ então
    $f(t_1,...,t_n) \in \mathcal{T}_{\Sigma}$.
...
- Para definirmos a sintaxe de fórmulas, vamos considerar um conjunto
  $\mathcal{P}$ de predicados (ou símbolos relacionais).

# Sintaxe

- Associamos a aridade de predicados usando uma função
  $a : \mathcal{P} \to \mathbb{N}$.
...
- Uma variável é dita ser livre em uma fórmula $\varphi$ se esta não possui um
  quantificador que a define.
...
- Para evitar a captura de variáveis, supomos um renomeamento de variáveis
  ligadas.
...
- A operação de substituição será utilizada tanto para a definição da semântica
  quanto da dedução natural para lógica de predicados.

# Semântica

# Semântica

- A partir de uma assinatura $\Sigma$ e um conjunto de predicados $\mathcal{P}$,
  o significado de uma fórmula $\varphi$ é definido em termos de uma
  _estrutura_.
...
- Uma estrutura é uma tripla $(M,F,R)$ em que:
  ...
  - $R$ um conjunto de relações $[\![P]\!]\subseteq M^n$ para cada predicado
    $P$.

# Semântica

- A partir de uma estrutura, podemos definir uma função de interpretação para o
  significado de termos e fórmulas.

# Semântica

- Primeiro, vamos definir a função $[\![t]\!]^k : M^k\to M$ semântica de termos,
  em que:
  - $k \in \mathbb{N}$
...
- Usando a semântica de termos, podemos definir a semântica de fórmulas da
  lógica de predicados como um subconjunto de $M^k$
...
- Em especial, para fórmulas fechadas, temos que
  $[\![\varphi]\!]^0 \subseteq \{()\}$.
...
- De posse da semântica da lógica de predicados, podemos estudar os formalismos
  de dedução natural e cálculo de sequentes.

# Dedução Natural

# Dedução Natural

- A dedução natural para lógica de predicados inclui regras para manipulação de
  quantificadores.
...
- Com essas 4 regras, conseguimos deduzir quaisquer fatos demonstráveis usando a
...
- Assim como a dedução natural, o cálculo de sequentes para a lógica de
  predicados apenas inclui regras para lidar com os quantificadores.
